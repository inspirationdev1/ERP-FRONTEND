...
import { customerSchema } from "../../../yupSchema/customerSchema";
import CustomerCardAdmin from "../../utility components/customer card/CustomerCard";
...
export default function Customers() {
  ...
  const handleEdit = (id) => {
    setEdit(true);
    axios
      .get(`${baseUrl}/customer/fetch-single/${id}`)
      .then((resp) => {
        const data = resp.data.data;
        // const classId = data?.customer_class?._id || data.customer_class;
        // const matchedClass = customerClass.find((c) => c._id === classId);
  ...
  const Formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      if (isEdit) {
        ...
        axios
          .patch(`${baseUrl}/customer/update/${editId}`, fd)
          .then((resp) => {
            setMessage(resp.data.message);
            setType("success");
            handleClearFile();
        ...
        axios
          .post(`${baseUrl}/customer/register`, fd)
          .then((resp) => {
            setMessage(resp.data.message);
            setType("success");
            Formik.resetForm();
  ...
  const fetchCustomers = () => {
    axios
      .get(`${baseUrl}/customer/fetch-with-query`, { params })
      .then((resp) => {
        setCustomers(resp.data.data);
        setNoofcustomers(resp.data.data.length);
      })
      .catch(() => console.log("Error in fetching customers data"));
  };
  ...
  const saveDocumentAttachment = async (values) => {
    ...
    Object.keys(values).forEach((key) => fd.append(key, values[key]));
    axios
      .post(`${baseUrl}/customer/document-attachment/${editId}`, fd)
      .then((resp) => {
  ...
  const deleteDocumentAttachment = async (id, index) => {
    axios
      .delete(`${baseUrl}/customer/delete-document-attachment/${id}`)
      .then((resp) => {
        removeRow(index);
        setMessage(resp.data.message);
        setType("success");
      ...
      <Box>
        <Box sx={{ borderBottom: 1, borderColor: "divider", mb: 2 }}>
          ...
          >
            ...
            <Tab label={isEdit ? "Edit Customer-1" : "Add New Customer-1"} />
            {/* <Tab label={isEdit ? "Edit Customer-2" : "Add New Customer-2"} /> */}
            <Tab label="View List" />
          </Tabs>
        </Box>
            ...
            <Paper sx={{ padding: "20px", margin: "10px" }}>
              ...
              >
                <Grid container spacing={2}>
                  {/* Customer Image Full Row */}
                  <Grid item xs={12}>
                    <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                      <Typography variant="h6">Customer Pic</Typography>
                        ...
                        >
                          ...
                          {value?.customer_image && (
                            <Button
                              ...
                              onClick={() =>
                                viewUploadFile(value?.customer_image)
                              }
                            >
                              View Pic
                            </Button>
