...
export default function Expenses() {
    ...
    axios
      ...
      .then((resp) => {
        ...
        Formik.setFieldValue(
          "expenseDate",
          resp.data.data.expenseDate
            ? dayjs(resp.data.data.expenseDate).format("YYYY-MM-DD")
            : "",
        );
        Formik.setFieldValue(
          "expenseTime",
        ...
        Formik.setFieldValue(
          "expenseAmount",
          resp.data.data?.expenseAmount || 0,
        );
  ...
  const handleChange = (index, field, value) => {
    ...
    if (field === "expensetype") {
      ...
      updated[index].tax_percent =
        updated[index]?.expensetype?.taxrate?.tax_percent;
    }

    const netAmount = updated[index]?.expensePrice || 0;
    const tax_percent = updated[index]?.tax_percent || 0;
    const taxtype = updated[index]?.taxtype || "inclusive";
    let taxable_amount = netAmount;
    updated[index].expenseAmount = netAmount;
    let tax_amount = 0;
