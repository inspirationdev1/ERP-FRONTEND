...
import { itemtypeSchema } from "../../../yupSchema/itemtypeSchema";

export default function Itemtype() {
  const [studentItemtype, setStudentItemtype] = useState([]);
  ...
  const handleDelete = (id) => {
    if (confirm("Are you sure you want to delete?")) {
      axios
        .delete(`${baseUrl}/itemtype/delete/${id}`)
        .then((resp) => {
  ...
  const handleEdit = (id) => {
    ...
    setEdit(true);
    axios
      .get(`${baseUrl}/itemtype/fetch-single/${id}`)
      .then((resp) => {
        Formik.setFieldValue("itemtype_name", resp.data.data.itemtype_name);
        Formik.setFieldValue("itemtype_code", resp.data.data.itemtype_code);
  ...
  const Formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      if (isEdit) {
        ...
        axios
          .patch(`${baseUrl}/itemtype/update/${editId}`, {
            ...values,
          })
          .then((resp) => {
        ...
        axios
          .post(`${baseUrl}/itemtype/create`, { ...values })
          .then((resp) => {
  ...
  const fetchstudentsitemtype = () => {
    axios
      .get(`${baseUrl}/itemtype/fetch-all`)
      .then((resp) => {
        console.log("Fetching data in  Casting Calls  admin.", resp);
        setStudentItemtype(resp.data.data);
      })
      .catch((e) => {
      ...
      <Box>
        <Box sx={{ borderBottom: 1, borderColor: "divider", mb: 2 }}>
          ...
          >
            ...
            <Tab label={isEdit ? "Edit Itemtype" : "Add New Itemtype"} />
            <Tab label="View List" />
          </Tabs>
        </Box>
            ...
            <Paper sx={{ padding: "20px", margin: "10px" }}>
              ...
              >
                <TextField
                  ...
                  variant="outlined"
                  name="itemtype_name"
                  value={Formik.values.itemtype_name}
                  onChange={Formik.handleChange}
                  onBlur={Formik.handleBlur}
                />
                {Formik.touched.itemtype_name &&
                  Formik.errors.itemtype_name && (
                ...
                <TextField
                  ...
                  variant="outlined"
                  name="itemtype_code"
                  value={Formik.values.itemtype_code}
                  onChange={Formik.handleChange}
                  onBlur={Formik.handleBlur}
                />
                {Formik.touched.itemtype_code &&
                  Formik.errors.itemtype_code && (
                    <p style={{ color: "red", textTransform: "capitalize" }}>
                      {Formik.errors.itemtype_code}
                    </p>
                  )}
                ...
                <TableBody>
                  {studentItemtype.map((value, i) => (
                    ...
                    >
                      ...
                      <TableCell align="right">{value.itemtype_code}</TableCell>
                      <TableCell align="right">
                        {(value?.taxrate?.tax_percent || "0") + " %"}
                      </TableCell>
                      <TableCell align="right">
                        {(value?.taxrate?.taxtype || "inclusive") + " %"}
                      </TableCell>
                      <TableCell align="right">
                        <Box
                          sx={{
