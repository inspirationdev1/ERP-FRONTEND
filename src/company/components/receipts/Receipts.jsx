...
export default function Receipts() {
  ...
  const handleEdit = async (id) => {
    ...
    setEdit(true);
    axios
      .get(`${baseUrl}/receipt/fetch-single/${id}`)
      .then((resp) => {
        Formik.setFieldValue("receiptCode", resp.data.data.receiptCode);
        Formik.setFieldValue(
          "receiptDate",
          resp.data.data.receiptDate
            ? dayjs(resp.data.data.receiptDate).format("YYYY-MM-DD")
            : "",
        );
        Formik.setFieldValue(
          "receiptTime",
  ...
  const handleChange = (index, field, value) => {
    ...
    if (field === "siId") {
      ...
      const invBal =
        (updated[index].siId.totalNetAmount || 0) -
        (updated[index].siId.totalPaidAmount || 0);
                    ...
                    {receiptDetails.map((row, index) => (
                      ...
                      >
                        ...
                        <Autocomplete
                          disabled={row.isEdit}
                          options={
                            Array.isArray(salesinvoices) ? salesinvoices : []
                          }
