...
export default function PaymentPrint() {
  ...
  useEffect(() => {
    if (reportHeader?.company_image) {
      getBase64Image(`${reportHeader.company_image}`).then(setLogo);
    }
  }, [reportHeader]);
  ...
  useEffect(() => {
    const fetchPrintPayment = async () => {
      try {
        const paymentPrintResponse = await axios.get(
          `${baseUrl}/payment/fetch-print/${id}`,
          { params: { id: id } },
        );
        ...
        const rptHeader = {
          company_name: paymentPrintResponse.data.data?.company.company_name,
          address: paymentPrintResponse.data.data?.company.address,
          city: paymentPrintResponse.data.data?.company.city,
          state: paymentPrintResponse.data.data?.company.state,
          country: paymentPrintResponse.data.data?.company.country,
          company_image: paymentPrintResponse.data.data?.company.company_image,
        };
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.headerContainer}>
          ...
          <View style={styles.companyInfo}>
            <Text style={styles.companyName}>{reportHeader?.company_name}</Text>
        ...
        {/* 🔷 Title */}
        <Text style={styles.reportTitle}>FEES PAYMENT</Text>
        {/* <View style={styles.header}>
                    ...
                    <View style={styles.spaceY}>
                        <Text style={styles.textBold}>{printPayment.company.company_name}</Text>
                        <Text style={styles.textBold}>{printPayment.company.address} - {printPayment.company.city}</Text>
                        <Text style={styles.textBold}>{printPayment.company.state} - {printPayment.company.country}</Text>
        ...
        <View>
          ...
          <View style={styles.rowStyle}>
            ...
            <Text style={styles.valueStyle}>{printPayment.status}</Text>

            <Text style={styles.labelStyle}>Remarks :</Text>
            <Text style={styles.valueStyle}>{printPayment.remarks}</Text>
          </View>
        </View>
        ...
        <Table style={styles.table}>
          ...
          <TR key={100}>
            ...
            <TD style={styles.td}>
              <Text style={styles.rightText}>
                {formatAmount(printPayment.totalexpenseAmount)}
              </Text>
            </TD>

            <TD style={styles.td}>
              <Text style={styles.rightText}>
                {formatAmount(printPayment.totalpaidAmount)}
              </Text>
            </TD>
          </TR>
        </Table>
